...
function openWeexWithdraw2FAModal() {
  ...
  if (display) display.textContent = `${amt.toFixed(2)} USDT`;
  if (modal) modal.style.display = 'flex';
  playSound('click');
}
...
function sendSimulatedEmailCode() {
  ...
  showOrderToast('info', 'Verification Code Sent', 'Simulated 6-digit code sent to your registered security email.');
...
async function executeVerifiedWithdrawal() {
  ...
  if (!code || code.length < 6) {
    showOrderToast('error', 'Invalid 2FA Code', 'Please enter your 6-digit Google Authenticator code.');
    return;
  }
  ...
  try {
    ...
    if (data.success || data.status === 'success') {
      ...
      showOrderToast('success', 'Withdrawal Broadcasted', `-${formatNumber(amt, 2)} USDT transferred to ${addr.substring(0, 10)}... via ${net}.`);
      playSound('order_fill');
...
function renderWeexHoldingsTable() {
  ...
  const holdings = [
    ...
    { asset: 'OMNI', name: 'Omni Network', net: 'Arbitrum One', balance: 13450.00, avail: 13450.00, inOrder: 0.00, price: omniInfo.price, change: omniInfo.change },
